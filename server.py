...
def GetCommand(name, database):
  ...
  if not result: return "Invalid key"
  return result
...
def DumpCommand(database):
  ...
  Returns:
    ...
    then the string describes the error.
  """
  with open('storage.csv', mode='w') as csv_file:
    fieldnames = ['key','value', 'time']
    writer = library.csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()
    for key in database.store:
      writer.writerow({'key':key, 'value':database.store[key]['value'], 'time':database.store[key]['time']})
